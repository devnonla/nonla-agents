...
export function ChatToolCall({ toolName = "Tool", label, toolInput, toolOutput, toolError, running = false, icon, assistantLabel, assistantColor = "var(--primary)", showAvatar = false, className, defaultOpen = false }: ChatToolCallProps) {
  ...
  const header = (
    <>
      <span className="relative size-4 shrink-0">
        <span className={cn("absolute inset-0 flex items-center justify-center transition-opacity", expandable && open ? "opacity-0" : "opacity-100", expandable && "group-hover:opacity-0")}>{statusIcon}</span>
        {expandable ? (
    ...
    <div className={cn("nonla-chat-tool mt-1", className)}>
      {showAvatar && assistantLabel ? (
        <div className="flex items-center gap-2.5 px-4 pt-3 pb-1">
          <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-bold tracking-wider uppercase select-none" style={{ background: assistantColor, color: "var(--primary-foreground)", letterSpacing: "0.08em" }}>
            {assistantLabel}
          </span>
        </div>
      ) : null}
        ...
        <div className={cn("mb-1 overflow-hidden rounded-lg border", hasError ? "border-destructive/35" : "border-border-subtle")}>
          ...
          {open && expandable ? (
            <div className="border-t border-border-subtle font-mono text-[11px]">
              ...
              {!isPending ? (
                <pre className={cn("m-0 max-h-75 overflow-y-auto px-3 py-2 break-all whitespace-pre-wrap font-normal leading-[1.65] text-muted-foreground", hasInput && "border-t border-border-subtle", hasError ? "bg-destructive/6" : "bg-muted/40")}>
