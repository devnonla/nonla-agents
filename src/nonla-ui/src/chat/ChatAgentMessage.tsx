...
export function ChatAgentMessage({
  ...
  return (
    <div className={cn("nonla-chat-agent mt-1", className)}>
      {thinking ? (
        <ChatThinking thinking={thinking} duration={thinkingDuration ?? 0} streaming={thinkingStreaming} />
      ) : null}
      {body ? (
        <div className="min-w-0 px-4 pb-0.5">
          <div className="nonla-chat-body text-(length:--chat-body-size) leading-(--chat-body-leading) text-[#ececec] antialiased">
            {body}
          </div>
        </div>
      ) : null}
