...
function DefaultAvatar({ name }: { name: string }) {
  ...
  return (
    <div className="flex size-16 items-center justify-center rounded-full bg-brand/20 text-xl font-semibold text-brand-soft" aria-hidden>
      {initial}
    </div>
  );
}

export function ChatWelcome({ name, description, avatar, modelLabel, toolCount = 0, starters = [...DEFAULT_STARTERS], onStarter, disabled, className }: ChatWelcomeProps) {
