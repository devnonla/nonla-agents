...
export function MarkdownTable({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative my-4 overflow-hidden rounded-lg border border-border-subtle group">
      <button
        type="button"
        onClick={handleCopy}
      ...
      >
        {copied ? <ClipboardCheckIcon size={12} /> : <ClipboardIcon size={12} />}
      </button>

      <div className="overflow-x-auto">
        <table
          ref={tableRef}
          className="w-full border-separate border-spacing-0 text-[14px] [&_th]:bg-foreground/[0.07] [&_th]:text-foreground [&_th]:font-semibold [&_th]:px-2.5 [&_th]:py-1.5 [&_th]:text-left [&_th]:border-r [&_th]:border-b [&_th]:border-border-subtle [&_th:last-child]:border-r-0 [&_td]:px-2.5 [&_td]:py-1.5 [&_td]:align-top [&_td]:text-foreground [&_td]:border-r [&_td]:border-b [&_td]:border-border-subtle [&_td:last-child]:border-r-0 [&_tbody_tr:last-child_td]:border-b-0"
        >
          {children}
        </table>
      </div>
