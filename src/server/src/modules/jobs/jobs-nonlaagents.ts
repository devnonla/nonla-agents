/**
 * TS nonlaagents client written into each job workspace under `node_modules/@nonla-agents/runtime`
 * (and, for back-compat, the legacy `node_modules/nonlaagents`). Talks to the localhost proxy started by jobs-runner.
 */

export const JOBS_NONLAAGENTS_INDEX_TS = `const url = process.env.NONLAAGENTS_URL ?? "";
...
export const JOBS_NONLAAGENTS_PACKAGE_JSON = `{
...
/** Legacy bare specifier — kept so jobs saved before the @nonla-agents/runtime rename still resolve. */
export const JOBS_NONLAAGENTS_LEGACY_PACKAGE_JSON = `{
  "name": "nonlaagents",
  "version": "0.0.0",
  "type": "module",
  "main": "index.ts"
